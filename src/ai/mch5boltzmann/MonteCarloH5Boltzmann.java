 ...
 *
 */
public class MonteCarloH5Boltzmann implements Engine {
	 ...
	 * @param root 
	 */
	public MonteCarloH5Boltzmann(Board board, String color, int moveNumber, 
			int allMovesNumber) {
		this.board = board;
		this.color = color;
	...
	private Node treePolicy(Node treePolicy_node, Board treePolicy_board) throws Exception {
		...
		while(numberNode< this.allMovesNumber) {
			...
			if(treePolicy_node.getUntriedMoves().size() != 0) {
				...
				//non-deterministic.
				try {
					treePolicy_node = bestChild(treePolicy_node);
				} catch(Exception e) {
	 ...
	 * @param board Board with initial position (for simulation).
	 * @return Winning side: "w" for white, "b" for black, "0" for draw.
	 * @throws Exception I don't remember. It has never occurred.
	 */
	private String defaultPolicy(Node node_defaultPolicy, Board defaultPolicy_board) throws Exception {
		Random generator = new Random();
		String color = node_defaultPolicy.getColor();
	 ...
	 * @param node Node amongst which children selection have to take place.
	 * @return Best node.s
	 */
	...
	private Node bestChild(Node node) {
		...
		for(Node child : node.getChildren()) {
			...
			childrenProbabilityH5( child,  sum,  t);
		}//end X.
		...
		//picked number.
		selectedChild = pickedNumberH5(organizedChildren, 
				 randomNumber,  bestFitProb,  selectedChild   );
		//Return selected child.
		return selectedChild;
	}
private void childrenProbabilityH5(Node child, double sum, double t) {
	...
	private Node pickedNumberH5 (List<Tuple<Double, Node>> organizedChildren, 
