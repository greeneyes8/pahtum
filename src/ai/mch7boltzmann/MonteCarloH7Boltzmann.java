...
/**
 * Monte-Carlo Tree Search with narrowed scope to the best 7 plays in
 ...
 *
 */
public class MonteCarloH7Boltzmann implements Engine {
	 ...
	 * @param root 
	 */
	public MonteCarloH7Boltzmann(Board board, String color, int moveNumber, 
			int allMovesNumber) {
		this.board = board;
		this.color = color;
	...
	private Node treePolicy(Node node_tp, Board board_tp) throws Exception {
		...
		while(numberNode < this.allMovesNumber) {
			...
			if(node_tp.getUntriedMoves().size() != 0) {
				...
				Node newNode =  node_tp.expand(board_tp, this.color);
	 ...
	 */
	private String defaultPolicy(Node node_dp, Board dp_board) throws Exception {
		Random generator = new Random();
		String color = node_dp.getColor();
		int moveNumber = node_dp.getMoveNumber();
		String w = "w";
		String b = "b";
		int valuenboltz_7 = 7;
		...
		while(moveNumber < this.allMovesNumber) {
			...
			//Narrow list of valid moves to the best 7 in accordance to the 
			//heuristic function.
			listValidMoves = dp_board.heuristic_bestX_moves(color, valuenboltz_7);
	...
	private Node bestChild(Node node) {
		...
		for(Node child : node.getChildren()) {
			...
			childrenProbability( child,  sum,  t);
		}//end X.
		...
		//picked number.
		selectedChild = pickedNumber(organizedChildren, 
				 randomNumber,  bestFitProb,  selectedChild   );
		//Return selected child.
		return selectedChild;
	}
	
private void childrenProbability(Node child, double sum, double t) {
	...
	private Node pickedNumber (List<Tuple<Double, Node>> organizedChildren, 
