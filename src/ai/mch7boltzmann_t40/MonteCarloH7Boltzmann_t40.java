 ...
 *
 */
public class MonteCarloH7Boltzmann_t40 implements Engine {
	 ...
	 * @param root 
	 */
	public MonteCarloH7Boltzmann_t40(Board board, String color, int moveNumber, 
			int allMovesNumber) {
		this.board = board;
		this.color = color;
	 ...
	 * @throws Exception 
	 */
	private Node treePolicy(Node node_tp, Board tp_board) throws Exception {
		...
		while(numberNode< this.allMovesNumber) {
			...
			if(node_tp.getUntriedMoves().size() != 0) {
				...
				tp_board.makeMove(node_tp.getMove(), color);
				numberNode = node_tp.getMoveNumber() ;
			}
		}
		return node_tp;
	 ...
	 */
	private String defaultPolicy(Node no_dedp, Board dp_board) throws Exception {
		Random generator = new Random();
		String color = no_dedp.getColor();
		int moveNumber = no_dedp.getMoveNumber();
		String w = "w";
		String b = "b";
		int valuebolt40_7 = 7;
		...
		while(moveNumber < this.allMovesNumber) {
			...
			//heuristic function.
			listValidMoves = dp_board.heuristic_bestX_moves(color, valuebolt40_7);
...
private void yChildT40(Node child, double sum, double t) {
